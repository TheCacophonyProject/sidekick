...
export interface DevicePlugin {
	...
	updateWifiNetwork(
		options: DeviceUrl & { ssid: string; password?: string },
	): Result;
...
const [DeviceProvider, useDevice] = createContextProvider(() => {
	...
	const startDiscovery = async () => {
		...
		for (const device of devices.values()) {
			...
			if (connection.success) {
				...
				console.log("device not connected", device);
	...
	const [isUpdating, setIsUpdating] = createSignal<DeviceId[]>([]);
	// Update
	...
	const intervals = new Map<DeviceId, NodeJS.Timeout>();

	createEffect(() => {
		for (const deviceId of isUpdating()) {
			if (intervals.has(deviceId)) continue;
			const interval = setInterval(async () => {
				const device = devices.get(deviceId);
				if (!device || !device.isConnected) return;
				const res = await CapacitorHttp.get({
					url: `${device.url}/api/salt-update`,
					headers: { ...headers, "Content-Type": "application/json" },
					webFetchExtra: {
						credentials: "include",
					},
				...
				if (res.status === 200) {
					...
					if (data.RunningUpdate === false) {
						...
						setIsUpdating((prev) => prev.filter((id) => id !== deviceId));
					}
				}
			}, 5000);
			intervals.set(deviceId, interval);
		}
	});

	const isDeviceUpdating = (deviceId: DeviceId) => {
		return isUpdating().includes(deviceId);
	};
