...
function WifiSettingsTab() {
	...
	const [wifiConnectedToInternet, {refetch: checkDeviceWifiInternetConnection}] = createResource(currentWifi(),async (wifi) => {
	...
	const [modemConnectedToInternet] = createResource(async () => {
			...
			<section class="w-full">
				...
				>
					<div class="flex h-full w-full items-center justify-between p-2">
						...
						<button class="bg-blue-500 text-white p-2 rounded-md" onClick={() => checkDeviceWifiInternetConnection()}>
							Check Connection
						</button>
					</div>
				</FieldWrapper>
...
function GeneralSettingsTab() {
	...
	return (
		<div class="flex w-full flex-col space-y-2 px-2 py-4">
			...
			<FieldWrapper type="text" value={groupName()} title="Group" />
			<A
