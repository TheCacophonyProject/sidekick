		...
		>
			<div class=" flex items-center justify-between">
				<div class="w-full" onClick={() => openDeviceInterface()} role="button">
					...
					<Show
						when={props.batteryPercentage !== "" && props.batteryPercentage}
					>
						{(percentage) => (
							<div class="mt-2 flex w-full items-center space-x-2 text-slate-700">
								<FaSolidBatteryFull size={20} />
								<p class="text-sm">
									Battery: {percentage() === "-1" ? "No reading" : percentage()}
									%
			...
			() => (
				<button
					onClick={async () => {
						...
						console.log("AP button clicked, current state:", context.apState());
						try {
					...
					disabled={["loadingConnect", "loadingDisconnect"].includes(
						context.apState(),
					)}
				...
				>
					<Show
						...
						fallback={<FaSolidSpinner size={20} class="animate-spin" />}
					>
					...
					<span class="font-medium sm:hidden">
						<Switch>
							<Match when={context.apState() === "connected"}>Disconnect</Match>
							<Match when={context.apState() === "disconnected"}>Connect</Match>
							<Match when={context.apState() === "loadingConnect"}>
								Connecting
							</Match>
							<Match when={context.apState() === "loadingDisconnect"}>
							...
							<Match when={context.apState() === "default"}>Connect</Match>
						</Switch>
					</span>
